=====================================================================================
   ...
   - Professional shell + logo support + public/app variants
   ===================================================================================== */
...
export type AppShellProps = {
  brand: { name: string; href?: string; logoSrc?: string; logoAlt?: string; subtitle?: string };
  nav: AppNavItem[];
...
function BrandMark({ brand }: { brand: AppShellProps["brand"] }) {
  const alt = brand.logoAlt ?? brand.name;
  const subtitle = brand.subtitle ?? BRAND.tagline;

  return (
    <a href={brand.href ?? "/"} className="flex items-center gap-3 font-extrabold tracking-tight">
      <img
        src={brand.logoSrc ?? BRAND.logoSrc}
        alt={alt}
        className="h-10 w-10 rounded-2xl bg-white p-1 ring-1 ring-black/10 shadow-sm object-contain"
        loading="eager"
      />
      <div className="leading-tight">
        <div className="text-sm sm:text-base">{brand.name}</div>
        <div className="text-[11px] font-semibold text-black/50">{subtitle}</div>
      </div>
    </a>
  );
}
...
export default function AppShell({
      ...
      {nav.map((item) => (
        <li key={item.to}>
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "flex items-center gap-3 rounded-2xl px-3 py-2 text-sm font-extrabold outline-none",
                "transition-colors focus-visible:ring-2 focus-visible:ring-black/30",
                isActive ? "text-white shadow-sm" : "text-neutral-700 hover:bg-neutral-100"
              )
            }
            style={({ isActive }) =>
              isActive
                ? { background: `linear-gradient(135deg, ${BRAND.colors.blue}, ${BRAND.colors.blue2})` }
                : undefined
            }
          >
  ...
  const Sidebar = (
    <aside className="hidden md:flex md:w-72 md:flex-col md:border-r md:bg-white/80 md:backdrop-blur">
  ...
  const Header = (
    <header
      className={cn(
        ...
        variant === "public" ? "text-white" : "bg-white/80 backdrop-blur"
      )}
      style={
        variant === "public"
          ? { background: `linear-gradient(135deg, ${BRAND.colors.blue}, ${BRAND.colors.blue2})` }
    ...
    >
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:absolute focus:left-3 focus:top-3 focus:z-50 focus:rounded-2xl focus:bg-white focus:px-3 focus:py-2 focus:ring-2 focus:ring-black/40"
      >
        Skip to content
      </a>
      ...
      <div className="w-full px-4 md:px-6 flex items-center gap-3">
        <button
          type="button"
          className={cn(
            "md:hidden rounded-2xl px-3 py-2 font-extrabold",
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive ? "bg-white text-[#0d2c54]" : "text-white/90 hover:bg-white/10"
                  )
                }
              >
                {i.label}
  ...
  if (variant === "public") {
    return (
      <div className="min-h-dvh bg-neutral-50 text-neutral-900">
        {MobileDrawer}
        {Header}
        <main id="main" className="mx-auto w-full max-w-6xl px-4 py-8 md:px-6">
          <Outlet />
        </main>

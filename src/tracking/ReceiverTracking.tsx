...
import { Truck, MapPin, Calendar, ChevronRight, ShieldCheck, Search } from "lucide-react";

type Props = { initialTrackingCode?: string };

export default function ReceiverTracking({ initialTrackingCode = "" }: Props) {
        ...
        <div className="absolute top-4 left-4 right-4">
          <div className="bg-white/95 backdrop-blur rounded-2xl shadow-soft border p-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs font-extrabold text-blue-700 uppercase tracking-wider">Tracking ID</div>
              <div className="mt-1 flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-full bg-transparent outline-none text-sm font-extrabold text-slate-800"
                />
              </div>
            </div>
            <div className="hidden sm:block text-right">
              <div className="text-xs font-extrabold text-blue-700 uppercase tracking-wider">Estimated</div>
              <div className="text-sm font-extrabold text-slate-900">Today, 2:00 PM</div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Code: <span className="font-mono font-bold">{trackingCode || "—"}</span>
              </p>
            </div>
            ...
            <div key={i} className="relative pl-6">
              <div
                className={[
                  ...
                  e.active ? "bg-green-500 ring-4 ring-green-100" : "bg-gray-300",
                ].join(" ")}
              />
              <h3 className={["font-bold", e.active ? "text-green-700" : "text-gray-700"].join(" ")}>{e.status}</h3>
              <p className="text-xs text-gray-400">
                {e.date} • {e.time}
              </p>
              {e.desc ? <p className="text-sm text-gray-600 mt-1 bg-gray-100 p-2 rounded">{e.desc}</p> : null}
            </div>
          ))}
        </div>
      </div>
      ...
      {showReschedule ? (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-white w-full sm:w-96 p-6 rounded-t-2xl sm:rounded-2xl animate-in slide-in-from-bottom">
            ...
            <p className="text-sm text-gray-500 mb-4">We will send an OTP to verify you are the owner.</p>
            <button className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl mb-3">Send OTP</button>
            <button onClick={() => setShowReschedule(false)} className="w-full text-gray-500 font-bold py-3">
              Cancel
            </button>
          </div>
        </div>

  ...
  return (
    <div className="p-6 space-y-6 animate-in fade-in">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-black text-[#0d2c54] uppercase tracking-tight">{t("Sorting Board")}</h1>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{parcels.length} {t("Parcels Pending")}</p>
        </div>
        <div className="flex gap-3">
          ...
          <button 
            onClick={handleSort}
            disabled={!bin || Object.values(selected).filter(Boolean).length === 0}
        ...
        {parcels.map(p => (
          <div 
            ...
            className={`p-5 rounded-[2rem] border-2 transition-all cursor-pointer ${
              selected[p.id] ? 'border-[#0d2c54] bg-blue-50/50 shadow-md' : 'border-gray-100 bg-white'
            }`}
          >
            <div className="flex justify-between items-start mb-2">
              <span className="font-mono text-xs font-bold text-gray-400">#{p.trackingId}</span>
              {selected[p.id] && <div className="w-5 h-5 bg-[#0d2c54] rounded-full flex items-center justify-center text-white"><CheckCircle size={12}/></div>}
            </div>
            <p className="font-black text-[#0d2c54]">{p.receiverName}</p>
            <p className="text-[10px] text-gray-400 font-bold uppercase mt-1 line-clamp-1">{p.deliveryAddress}</p>
          </div>
        ))}
      </div>
    </div>
